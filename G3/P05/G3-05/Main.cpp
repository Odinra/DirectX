...
bool Buttonizer()
{
	...
	butrect.right = 256;
	butrect.bottom = 33;
	D3DCOLOR butcolor = D3DCOLOR_ARGB(1, 255, 255, 255);
	D3DXVECTOR3 butpos(Width/2 - 100, Height/2 - 12.5, 0);

	spriteMap->Begin(NULL);
	spriteMap->Draw(buttonstex, &butrect, NULL, &butpos, butcolor);
	spriteMap->End();
...
bool Setup()
{
	...
	D3DXCreateTextureFromFile(Device, "Buttons.png", &buttonstex);
	srand(GetTickCount());
...
bool Display(float timeDelta)
{
	if( Device )
	{
		...
		{
			TheCamera.walk(400.0f * timeDelta);
